var table = NioApp.DataTable('#dt-table', {
    ...
    searchDelay: 500,
    ajax: {
        url: '/admin/master-management/size-chart/datatable',
        beforeSend: function () {
            blockUI();
        },
        complete: function () {
...
$('#form-data').submit(function(e) {
    ...
    $.ajax({
        url : "/admin/master-management/size-chart/store",  
        data : formData,
        type : "POST",
        dataType : "JSON",
        cache:false,
...
function edit(id) {
    $.ajax({
        url: '/admin/master-management/size-chart/edit/'+id,
        dataType: 'JSON',
        success: function(response) {
            if(response.status) {
                $('#modalForm').modal('show');
...
function hapus(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: '/admin/master-management/size-chart/delete/'+id,
                dataType: 'JSON',
                success: function(response) {
                    if(response.status){
                        $("#dt-table").DataTable().ajax.reload(null, false);
...
function activate(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: '/admin/master-management/size-chart/activate/'+id,
                dataType: 'JSON',
                success: function(response) {
                    if(response.status){
                        $("#dt-table").DataTable().ajax.reload(null, false);
...
function deactivate(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: '/admin/master-management/size-chart/deactivate/'+id,
                dataType: 'JSON',
                success: function(response) {
                    if(response.status){
                        $("#dt-table").DataTable().ajax.reload(null, false);
